...
load(strcat([root_dir 'Number_of_trials' filesep 'Mode' num2str(cfg_simulations.coupling_mode) filesep 'FPrate' filesep cfg_simulations.oscillation filesep cfg_simulations.oscillation '_FPrate_ntrials_POS_phase_outcome_coupling' num2str(cfg_simulations.poc_simulations_Ntrials) '.mat']), 'FPrate_ntrials_POS');
load(strcat([root_dir 'Number_of_trials' filesep 'Mode' num2str(cfg_simulations.coupling_mode) filesep 'FPrate' filesep cfg_simulations.oscillation filesep cfg_simulations.oscillation '_FPrate_ntrials_U2watson_phase_outcome_coupling' num2str(cfg_simulations.poc_simulations_Ntrials) '.mat']), 'FPrate_ntrials_watson');
...
m=plot(sensitivity_ntrials_MI, 'Color', colors{4}, 'Linewidth', 3); hold on;
xticks(1:length(cfg_simulations.levels_nTrials));
xticklabels({cfg_simulations.levels_nTrials});
lgd=legend([l p w m], 'Circ. log. regr.', 'POS', 'Watson', 'MI', 'Location','northeast');
yticks(0:0.1:1);
lgd.FontSize = 15;
ax = gca;
ax.FontSize = 16;
...
m=plot(FPrate_ntrials_MI, 'Color', colors{4}, 'Linewidth', 3); hold on;
xticks(1:length(cfg_simulations.levels_nTrials));
xticklabels({cfg_simulations.levels_nTrials});
ax = gca;
ax.FontSize = 16;
