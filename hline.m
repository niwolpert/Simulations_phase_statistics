function hhh=hline(y,in1,in2,fontsize)
...
% 8 November 2001
%
% Nicolai Wolpert 23/10/2019 added fontsize
...
if length(y)>1  % vector input
    ...
    if ~isempty(label)
        ...
        yunit=(y-yy(1))/yrange;
%         if yunit<0.2
            text(x(1)+0.02*(x(2)-x(1)),y+0.02*yrange,label,'color',get(h,'color'), 'FontSize', fontsize)
%         else
%             text(x(1)+0.02*(x(2)-x(1)),y-0.02*yrange,label,'color',get(h,'color'), 'FontSize', fontsize)
%         end
    end
